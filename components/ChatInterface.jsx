          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 flex items-center gap-1">
                {mode === 'audio' && <Volume2 size={12} className="text-blue-500" />}
          ...
          <div className="flex items-center gap-2">
            {currentQuestion && totalQuestions && (
              <div className="hidden md:flex flex-col items-end mr-4">
                ...
                <span className="text-sm font-bold text-slate-800">Question {currentQuestion} / {totalQuestions}</span>
              </div>
            )}
            
            {onSaveExit && (
               <Button variant="secondary" size="sm" onClick={onSaveExit} className="flex items-center gap-2">
                 <Save size={16} />
            ...
            {onEndSession && (
              <Button variant="outline" size="sm" onClick={onEndSession} className="flex items-center gap-2 border-red-200 text-red-600 hover:bg-red-50">
                <StopCircle size={16} />
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 md:px-6 space-y-2 scroll-smooth bg-slate-50">
        {messages.length === 0 && (
           <div className="h-full flex flex-col items-center justify-center text-slate-400 opacity-80">
             <p>{mode === 'audio' ? 'The interviewer will start speaking...' : 'Send a message to start...'}</p>
           </div>
        )}
        {messages.map((msg) => (
      ...
      <div className="p-4 md:p-6 bg-white border-t border-slate-200 transition-all duration-300">
        ...
        {activeInputMode === 'audio' && onSendAudio ? (
          ...
          <div className="relative">
            <form onSubmit={handleSubmit} className="relative max-w-4xl mx-auto flex items-end gap-3 bg-slate-50 p-2 rounded-xl border border-slate-200 focus-within:border-blue-500 focus-within:bg-white focus-within:shadow-md transition-all">
              <textarea
                ref={inputRef}
                value={input}
